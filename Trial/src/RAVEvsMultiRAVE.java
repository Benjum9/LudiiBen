...
public class RAVEvsMultiRAVE {

    private static final int NUM_TRIALS = 100;

    private static final String GAME = "Havannah.lud" ;

    private static final double thinkingTime = 10.0 ;

    public static void main(final String[] args) {

        String filename = "/home/I6256403/project/RAVEvsMultiRAVEHavannah5.txt";
        ...
        try (FileWriter fw = new FileWriter(filename, true);  // true will append to file, false (or omitted) will overwrite
            ...
            AI uct = MCTS.createUCTRAVE(0.4);
            //AI multi = MCTS.createMAST(0.8) ;
            //AI multi = MCTS.createMultiMast(0.4,false, 0.4) ;
            AI multi = MCTS.createMultiRave(0.4, false, 0.4) ;
            ...
            for (int j = 1; j < NUM_TRIALS + 1; j++) {
                ...
                for (int p = 1; p <= game.players().count(); ++p) {
                    ...
                    if (ais.get(p).friendlyName().startsWith("MultiRave") && ranking[p] == 1) {
                        numMulti = numMulti + 1;
                        // System.out.println("numUCT2 "+numUCT2);
                    }


                    if (ais.get(p).friendlyName().startsWith("rave")) {
