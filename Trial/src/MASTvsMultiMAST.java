...
public class MASTvsMultiMAST {
    ...
    public static void main(final String[] args) {
        ...
        String filename = "/home/I6256403/project/MASTvsMultiMASTBreakthrough5.txt";
        ...
        try (FileWriter fw = new FileWriter(filename, true);  // true will append to file, false (or omitted) will overwrite
            ...
            AI uct = MCTS.createMAST(0.6, 0.4);
            AI multi = MCTS.createMultiMast(0.6, false, 0.4) ;
            ...
            for (int j = 1; j < NUM_TRIALS + 1; j++) {
                ...
                for (int p = 1; p <= game.players().count(); ++p) {
                    ...
                    if (ais.get(p).friendlyName().startsWith("MAST") && ranking[p] == 1) {
                        numUCT = numUCT + 1;
                        //System.out.println("numUCT "+numUCT);
                    }
                    if (ais.get(p).friendlyName().startsWith("MultiMAST") && ranking[p] == 1) {
                        numMulti = numMulti + 1;
                        // System.out.println("numUCT2 "+numUCT2);
                    }


                    if (ais.get(p).friendlyName().startsWith("MAST")) {
